...
* Z-machine バージョン 1-5 対応（バージョン 3・5 は 34 本のストーリーファイルで検証済み）
* バイリンガル表示（英語 + 翻訳）
...
|:--|:--|
| 3（`.z3`） | テストセット中の全作品が動作 |
| 4（`.z4`） | 動作。タイマー付き入力は無視されます |
| 5（`.z5`） | テストセット中の全作品が動作 |
| 6（`.z6`） | 未対応。グラフィックと複数ウィンドウが必要です |
| 8（`.z8`） | 未対応 |
...
現在のバージョンは **0.5.0** です。リリース履歴は [CHANGELOG-jp.md](CHANGELOG-jp.md) を参照してください。
