...
* Z-machine バージョン 1-8 対応（34 本のストーリーファイルと CZECH 適合テストで検証済み）
* バイリンガル表示（英語 + 翻訳）
...
検証には [jeffnyman/zifmia](https://github.com/jeffnyman/zifmia/tree/master) に
収録されているストーリーファイルと適合テストを使用しています。各バージョンの作品に
加え、CZECH・TerpEtude の適合テストとその期待出力が含まれています。

34 本のストーリーファイルで検証しています。

CZECH 適合テストは完全に通ります（Passed 406 / Failed 0、期待出力と一致）。
...
| 5（`.z5`） | 8 | 全て動作 |
| 6（`.z6`） | 2 | Zork Zero を含めテキストとして動作。グラフィックは非対応で、上部ウィンドウはピクセル配置ではなくステータスバーとして描画されます |
| 8（`.z8`） | 1 | 動作 |
...
* [jeffnyman/zifmia](https://github.com/jeffnyman/zifmia/tree/master) — 本インタプリタの検証に使用したストーリーファイルと適合テスト。CZECH・TerpEtude とその期待出力を含む
